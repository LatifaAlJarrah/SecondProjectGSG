...
export default function CategoryCard({ category, active, onClick }: Props) {
	return (
		<button
			...
			className={`flex h-28 w-full items-center justify-center rounded-md border transition ${
				active
		...
		>
			<div className="flex flex-col items-center gap-3">
				<span className={`text-2xl ${active ? "text-white" : "text-neutral-700"}`}>{category.icon}</span>
